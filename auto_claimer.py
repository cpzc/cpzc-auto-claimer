...
def run_login_flow(claimer):
    """Handle login + edit profile setup, returns True on success."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    while True:
        login_menu = (
            "   LOGIN   \n"
            "-----------\n"
            "[1] Manual Login (QR/Phone/Email)\n"
            "[2] Load Cookies (JSON file)\n"
            "[3] Load Cookies (sessions.txt)\n"
            "[4] Create New Account\n"
            "[5] Login with email:password file"
        )
        draw_border(login_menu)
        login_choice = input(f"\n{Fore.YELLOW}>> {Fore.WHITE}").strip()

        if login_choice == "4":
            if create_account(claimer.driver):
                logged_in = True
                break
            continue

        if login_choice == "5":
            logged_in = claimer.login_with_credentials(os.path.join(script_dir, "data", "accounts.txt"))
            if logged_in:
                break
        ...
        logged_in = False
        if login_choice == "3":
            cookies_file = os.path.join(script_dir, "data", "sessions.txt")
            if os.path.exists(cookies_file):
                print(Fore.CYAN + f"📂 Loading from: {cookies_file}")
                logged_in = claimer.login_with_cookies(cookies_file)
            else:
                print(Fore.RED + f"❌ File not found: {cookies_file}")
                input(Fore.YELLOW + "\nPress Enter to return...")
                return False
        elif login_choice == "2":
            cookies_file = os.path.join(script_dir, "data", "cookies.json")
            if os.path.exists(cookies_file):
                logged_in = claimer.login_with_cookies(cookies_file)
            else:
                print(Fore.RED + f"❌ File not found: {cookies_file}")
    ...
    save = input(Fore.YELLOW + "\nSave session cookies for next time? (y/n): " + Fore.WHITE).strip().lower()
    if save == 'y':
        claimer.save_cookies(os.path.join(script_dir, "data", "sessions.txt"))

    if not claimer.initialize_edit_profile_setup():
        print(Fore.RED + "❌ Edit Profile setup failed")
        input(Fore.YELLOW + "\nPress Enter to return...")
        return False
    return True

def generate_usernames():
    """Generate random username strings with options."""
    os.system("cls")
    print(Fore.CYAN + TITLE_ART)
    draw_border("   GENERATE USERNAMES   \n------------------------\nRandom letter/number strings")

    chars = input(Fore.YELLOW + "Character set (l=letters, n=numbers, b=both, default b): " + Fore.WHITE).strip().lower() or 'b'
    min_len = input(Fore.YELLOW + "Min length (default 4): " + Fore.WHITE).strip()
    min_len = int(min_len) if min_len.isdigit() else 4
    max_len = input(Fore.YELLOW + "Max length (default 6): " + Fore.WHITE).strip()
    max_len = int(max_len) if max_len.isdigit() else max(6, min_len)
    count = input(Fore.YELLOW + "How many to generate (default 1000): " + Fore.WHITE).strip()
    count = int(count) if count.isdigit() else 1000
    prefix = input(Fore.YELLOW + "Prefix (optional, press Enter for none): " + Fore.WHITE).strip()
    ...
    print(Fore.CYAN + f"\nGenerating {count} usernames ({label}, {min_len}-{max_len} chars)...")
    if prefix:
        print(Fore.CYAN + f"Prefix: '{prefix}'")

    save = input(Fore.YELLOW + "\nSave to file? (y/n, default n): " + Fore.WHITE).strip().lower()
    if save == 'y':
        path = input(Fore.YELLOW + "Filename (default data/usernames.txt): " + Fore.WHITE).strip() or os.path.join("data", "usernames.txt")
        try:
            with open(path, 'w') as f:
                for _ in range(count):
                    ...
                    f.write(u + '\n')
            print(Fore.GREEN + f"Saved {count} usernames to {path}")
        except Exception as e:
            print(Fore.RED + f"Error saving: {e}")
    else:
        usernames = []
        for _ in range(count):
            length = random.randint(min_len, max_len)
            u = prefix + ''.join(random.choices(pool, k=length))
            usernames.append(u)
        print(Fore.CYAN + f"\nGenerated {count} usernames (first 20 shown):")
        for u in usernames[:20]:
            print(Fore.WHITE + f"  {u}")
        if count > 20:
            print(Fore.CYAN + f"  ... and {count - 20} more")
    ...
    while True:
        ...
        if choice == "2":
            ...
            settings_text = (
                "   SETTINGS   \n"
                "--------------\n"
                "Current config loaded from config.json\n"
                "Edit config.json to change settings."
            )
            draw_border(settings_text)
            headless_val = "ON" if config.get("headless") else "OFF"
            print(Fore.CYAN + f"\n  [1] Username Input Delay: {config['username_input_delay']}s")
            print(Fore.CYAN + f"  [2] Headless Mode: {headless_val}")
            print(Fore.WHITE + f"\n  Enter number to toggle/edit, or press Enter to return")
            ...
            if setting_choice == "2":
                ...
                status_msg = f"Headless mode set to {'ON' if config['headless'] else 'OFF'}"

            continue

        # Option 1 needs browser
        claimer = TikTokSeleniumClaimer(
            headless=config.get("headless", False),
            username_input_delay=config["username_input_delay"],
        )

        if not claimer.setup_driver():
            print(Fore.RED + "❌ Failed to setup browser.")
            input(Fore.YELLOW + "\nPress Enter to return...")
            status_msg = "❌ Browser setup failed"
            continue

        if not run_login_flow(claimer):
            claimer.cleanup()
            status_msg = "❌ Login failed or cancelled"
            continue

        try:
            completed_msg = ""
            if choice == "1":
                update_title("Auto Scan & Claim")
                os.system("cls")
                script_dir = os.path.dirname(os.path.abspath(__file__))
                local_file = os.path.join(script_dir, "data", "usernames.txt")
                if os.path.exists(local_file):
                    claimer.auto_scan_and_claim_mode(local_file, config)
                else:
                    print(Fore.RED + "❌ No usernames.txt found")
                completed_msg = "✅ Auto scan finished"
